...
async function main() {
   ...
   * =================================================================================== */
  {
    ...
    sendJ(host, { type: "host", protocolVersion: 2, name: "Host", n: 4, teams: false, seats });
    const created = await nextMsg(host, (m) => m.type === "created");
    const code = created.code;
    sendJ(g1, { type: "join", protocolVersion: 2, code, name: "G1" });
    ...
    sendJ(g2, { type: "join", protocolVersion: 2, code, name: "G2" });
    ...
    sendJ(late, { type: "join", protocolVersion: 2, code, name: "Late" });
   ...
   * =================================================================================== */
  {
    ...
    const created = await nextMsg(host, (m) => m.type === "created");
    const code = created.code;
    sendJ(guest, { type: "join", protocolVersion: 2, code, name: "Guest" });
    ...
    sendJ(reGuest, { type: "rejoin", protocolVersion: 2, code, playerId: joined.playerId, token: joined.token });
    ...
    sendJ(reclaimer, { type: "reclaim", protocolVersion: 2, code, name: "Guest" });
  ...
  // "cpu" by the time G.over flips - proving the departed human gets no win/game credit.
  {
    ...
    sendJ(host, { type: "host", protocolVersion: 2, name: uniqueName, n: 4, teams: false, seats });
    const created = await nextMsg(host, (m) => m.type === "created");
    sendJ(host, { type: "start", protocolVersion: 2 });
    await nextMsg(host, (m) => m.type === "readyCheck");
    sendJ(host, { type: "readyUp" });
